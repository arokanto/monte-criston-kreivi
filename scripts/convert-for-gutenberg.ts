...
const htmlOutDir = new URL("../gutenberg-html/", import.meta.url);
const textOutDir = new URL("../gutenberg-text/", import.meta.url);

const bookSlug = "monte-criston-kreivi";
const bookTitle = "Monte-Criston kreivi";
const cssFiles = ["gutenberg.css", "book.css"];
...
async function writeCombinedOutputs(opf: OpfSpine, css: string): Promise<void> {
  ...
  const masterHtml = renderHtmlDocument({
    attrs: 'class="x-ebookmaker x-ebookmaker-3"',
    html: bodyHtml,
    lang: "fi",
    title: bookTitle,
  }, css);
